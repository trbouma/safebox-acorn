...
| Accepted-token swap interruption | A mint can consume incoming token proofs while the HTTP response carrying replacement signatures is lost. Mints without NUT-09 cannot automatically restore those signatures, and a relay failure can still prevent final proof persistence. | Before `/v1/swap`, persist and verify encrypted output secrets and blinding factors in the relay-backed `pending_swaps` record. Recover interrupted responses with NUT-09 without resubmitting inputs; refuse unrelated swaps while recovery is unresolved; retire the intent only after replacement proofs are relay-verified. Use mints that advertise NUT-09 and do not introduce application-local proof state. |
| Outgoing-transfer crash window | An interruption between bearer-token issuance and gift-wrapped transfer publication can create uncertainty or unsafe retry behavior. A complete durable transfer outbox remains a release gate. | Expose typed `RetryablePreSwapError` and `AmbiguousSwapError` boundaries so supervising applications retry only operations proven not to have reached the mint; use small test amounts; preserve transaction evidence; implement the roadmap's idempotent transfer outbox and acknowledged delivery state before pilot release. |
